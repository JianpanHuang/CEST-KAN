...
gs = 9
repeat_model = 5                                                                           
root = root = 'D:/projects/CEST-KAN0725/cestkanpc10/'
...
with open(record_save_path, mode='w', newline='') as file:
        ...
        writer.writerow(['model_name', 'val_loss','val_correlation1', 'val_correlation2', 'val_correlation3', 'val_correlation4','val_correlation5','val_correlation6','val_correlation7','val_correlation8','val_correlation9','test_loss', 'correlation1', 'correlation2', 'correlation3', 'correlation4','correlation5','correlation6','correlation7','correlation8','correlation9'])
...
for train_index, val_index in kf.split(data_arr):
    ...
    for noise in noise_level:
        ...
        valloader = DataLoader(val_dataset, batch_size=val_data_tensor.size(0), shuffle=False)
        ...
        for k1 in [9]:  # gs
            for k2 in [3]:   
                ...
                for i in range (1,repeat_model+1):  
                    ...
                    name = f"kan5_kf{k_fold_id}_hidden138_noise{noise}_r{i}" # gamma extra   name = f"lorentzkan5_{k1}_{k2}_{i}" # gamma extra
                    ...
                    for net_layers in net_layers_list:
                        ...
                        if best_model_state is not None:
                            torch.save(best_model_state, save_model_dir +'kf'+str(k_fold_id)+'_'+ str(hidd_layer_sz) + '_' + str(hidd_layer_num) + '_' + str(best_epoch) + '_' + str(int(train_time)) + ".pth")
                        else:
                            torch.save(model.state_dict(), save_model_dir +'kf'+str(k_fold_id)+'_'+ str(hidd_layer_sz) + '_' + str(hidd_layer_num) + '_' + str(epoch+1) + '_' + str(int(train_time)) + ".pth")
                        ...
                        val_labels_all_tensor = np.squeeze(torch.FloatTensor(val_labels_all))
                        val_outputs_all_tensor = np.squeeze(torch.FloatTensor(val_outputs_all))
                    
                    
                    
                        val_correlation = []
                        for n in range(val_labels_all_tensor.size(1)): #labels_all_tensor.size(1)
                            val_correlation.append(np.corrcoef(val_labels_all_tensor[:,n], val_outputs_all_tensor[:,n])[0, 1])   
                    
                        val_correlation = np.array(val_correlation)
                    
                   
                        all_model_val_loss.append(val_loss_ave)
                        all_model_val_correlation.append(val_correlation)
                        ...
                        with open(record_save_path, mode='a', newline='') as file:
                            writer = csv.writer(file)
                            writer.writerow([name,val_loss_ave,val_correlation[0],val_correlation[1],val_correlation[2],val_correlation[3],val_correlation[4],val_correlation[5],val_correlation[6],val_correlation[7],val_correlation[8],test_loss_ave,correlation[0],correlation[1],correlation[2],correlation[3],correlation[4],correlation[5],correlation[6],correlation[7],correlation[8]])
