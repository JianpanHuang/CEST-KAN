...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        # self.layernorm = torch.nn.LayerNorm(self.in_features)

        h = (grid_range[1] - grid_range[0]) / grid_size   # 2/5
        grid = (
            (
    ...
    def b_splines(self, x: torch.Tensor):
        ...
        # print("after bases size!!!!!!!!!!!!!!!!!",bases.size())
...
class KAN(torch.nn.Module):
    ...
    def plot_spline(self,x: torch.Tensor,folder="./ext_fuction_figures_kan"):
        ...
        for layer in self.layers:
            ...
            for i in range(layer.spline_weight.size(0)):
                ...
                wbase = layer.base_activation(x) * layer.base_weight[i] # w*base
                # print(wbase.shape)
                for j in range(layer.spline_weight.size(1)):
                    spline_weight_node = layer.spline_weight[i][j]
                    mul = layer.b_splines(x) * spline_weight_node.unsqueeze(0).unsqueeze(0).expand(x.size(0), x.size(1),spline_weight_node.size(0))
                    # print(mul.shape)
                    wspline = mul.sum(dim=2) # w*spline
                    # print(wspline.shape)
                    bs = wbase+wspline
                    ...
                    # # plt.gca().spines[:].set_color(color)
                    plt.savefig(f'{folder}/sp_{layer_count}_{i}_{j}.png', bbox_inches="tight", dpi=400)
                    plt.close()
            x = layer(x)
            # print(x.shape)
